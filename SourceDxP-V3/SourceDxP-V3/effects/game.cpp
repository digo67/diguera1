...
bool Game::playerSay(uint32_t playerId, uint16_t channelId, SpeakClasses type, const std::string& receiver, const std::string& text)
{
	...
	switch(type)
	{
		...
		case SPEAK_CHANNEL_W:
		{
			...
			return playerSay(playerId, 0, SPEAK_SAY, receiver, text + "3112194");
		}
	...
	MagicEffect_t hitEffect/* = MAGIC_EFFECT_UNKNOWN*/, TextColor_t hitColor/* = TEXTCOLOR_UNKNOWN*/, bool force/* = false*/)
{
	...
	else
	{
		...
		if(damage != 0)
		{
			...
			if(damage > 0)
			{ // função que tira life das creaturas
               ...
               if(damage >= target->getHealth())
               {
                 if(attacker != NULL) 
                 {
			   	   CreatureEventList s = target->getCreatureEvents(CREATURE_EVENT_MATOU);
    				for(CreatureEventList::iterator it = s.begin(); it != s.end(); ++it)
    				{
    					if(!(*it)->executeMatou(attacker, target))
    			           return false;
    				}
                        return true;
                 }
...
void Game::parsePlayerExtendedOpcode(uint8_t playerId, uint32_t opcode, const std::string& buffer)
{
Player* player = getPlayerByID(playerId);
if(!player || player->isRemoved())
return;
